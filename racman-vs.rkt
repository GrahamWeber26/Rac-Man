...
; Constants, Images & Structs
    ...
    (define playArea (rectangle WORLDSIZE WORLDSIZE "solid" "black"))
    (define wallDot (circle (/ DIAM 10) "solid" "blue"))
    (define wallBox (rectangle PATHWIDTH PATHWIDTH "solid" (color 33 33 255)))
    (define dotPic (circle (/ DIAM 10) "solid" "pink"))
    (define cookiePic (circle (/ DIAM 5) "solid" "yellow"))
    (define racman-pics
      (list
          (bitmap "images/basic-r.png")
          (rotate 270 (bitmap "images/basic-r.png"))
          (flip-horizontal (bitmap "images/basic-r.png"))
          (rotate 90 (bitmap "images/basic-r.png"))))
    (define racman-open
...
; Maze Creation Functions
  ...
  (define (createMaze pathWidth)
    (filter mazebox? (for*/list ([i (in-range (length boardMap))]
                [j (in-range (length (first boardMap)))])
            (if (string=? (list-ref (list-ref boardMap i) j) "wall") (make-mazebox (box (+ (* j 2 pathWidth) pathWidth) (+ (* i 2 pathWidth) pathWidth))) "x"))))
...
; Render Functions
  ...
  ; renderGame
    ...
    (define (renderGame gs)
      (place-image (list-ref racman-pics 
                      (match 
                        (racman-curdir (gamestate-racman gs)) 
                        ["right" 0] ["down" 1] ["left" 2] ["up" 3] 
                        [else (match (racman-nexdir (gamestate-racman gs))
                              ["down" 1] ["left" 2] ["up" 3] [else 0])])) 
                  (racman-x (gamestate-racman gs)) (racman-y (gamestate-racman gs))
                  (renderDots (gamestate-dots gs) (renderMaze mehze playArea))))
...
; Move Functions
  ...
  ; mazeCollision
    ...
    ; renderMaze
      ...
      ; lomb, im -> image
      ; render the maze
      (define (renderMaze lomb im)
        (cond
          [(empty? lomb) im]
          [else (renderMaze (rest lomb)
                  (renderBox (mazebox-box (first lomb)) im))]))
